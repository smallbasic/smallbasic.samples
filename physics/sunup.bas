...
12 mode=0
20 GOSUB 300
21 '' Chandler Arizona Latitude=33N18 Longitude=-111W50
22 B5=33+(18/60) : L5=-1*(111+(50/60))
23 H=7
25 if mode=1 then goto 50
...
120 GOSUB 910: A(1)=A5: D(1)=D7
130 T=T+1
140 GOSUB 910: A(2)=A5: D(2)=D7
...
271 if mode!=1 then goto 280
272 PRINT "Latitude ";B5;"  Longitude ";L5*360;"  TimeZone ";H
273 PRINT "Day/Month/Year ";nd$
280 END
290 ''
300 ''        Constants
310 DIM A(2),D(2)
320 REM P1=3.14159265: P2=2*P1
320 P1=PI        : P2=2*P1
...
600 A1=2*V2-4*V1+2*V0: B=4*V1-3*V0-V2
610 D8=B*B-4*A1*V0: IF D8<0 THEN 800
620 D8=SQR(D8)
...
670 E=(-B+D8)/(2*A1)
680 IF E> 1 OR E< 0 THEN E=(-B-D8)/(2*A1)
...
1140 S=V/SQR(U):D7=ATN(S/SQR(1-S*S))
1150 R5=1.00021*SQR(U)
1160 RETURN
1165 ''
1170 ''     Calendar --> JD
1171 '' Get current Date 
1172 nd$=DATE$ 
1173 '' DATE$ - returns the current day as string "DD/MM/YYYY" 
1174 M=MID$(nd$,4,2)+0 
1175 Y=MID$(nd$,7,4)+0 
1176 Day=MID$(nd$,1,2)+0 
1177 if mode=1 then goto 1190
1180 INPUT "Year, Month, Day";Y,M,Day
1190 G=1: IF Y< 1583 THEN G=0
1200 D1=INT(Day): F=Day-D1-.5
1210 J=-INT(7*(INT((M+9)/12)+Y)/4)
1220 IF G=0 THEN 1260
1230 S=SGN(M-9): A3=ABS(M-9)
1240 J3=INT(Y+S*INT(A3/7))
