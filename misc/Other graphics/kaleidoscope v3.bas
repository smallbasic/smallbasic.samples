'kaleidoscope v3.bas 2015-08-31 adapted for SmallBASIC by MGA/B+ from
REM Adapted from Phix to Yabasic 2.769 by Galileo, 2015/08 640x520
'v3 with color modulation
randomize
...
sub color_modulator()
   if segment >= 650 then cls : segment = 0
   cm=segment mod 30
   if !cm then 
      red=int(rnd*2)
      green=int(rnd*2)
      blue=int(rnd*2)
      if !(red or green or blue) then red=1
   end if
   kcol = rgb(red*(255-cm*8),green*(255-cm*8),blue*(255-cm*8))     
   segment = segment + 1
end 
