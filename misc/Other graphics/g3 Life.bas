'g3 Life.bas SmallBASIC (not M$) B+ 
...
'g2 Bpf 2015-03-22 post with 123-28.jpg an=320
'g3 add RND seeder to GetSeed() menu 3 post Bpf 2015-03-23 
'g3 posted with 1234-4.jpg an=130 110X110 Rnd fill .5
...
an=130: s=int(ymax/an): if s<3 then cs=s else cs=s-1: bigBlock=an*s
...
sub getSeed()
  ...
  local menu, f, inps, cols, r, c
  color 0,15:cls
  'not local seed(), sdRow
  sdRow=0
  ?
  ? "                 *** THE GAME OF LIFE FOR BPF ***" 
  ? "         version G3 Life.bas in SmallBASIC(not M$)  by B+  2015-March"
  ?
  ...
  ?
  ? " type 3(enter) to seed Randomly, spec: rows, columns and density of fill
  ?
  ...
  if menu="1" then
    ...
    Input "For RND seed, enter the number of rows ";sdrow
    Input "For RND seed, enter the number of columns ";cols
    Input "Random density 0=empty 1=completely full, .5 half ";f
    If sdrow<=an and sdrow>0 and cols<=an and cols>0 and f>0 and f<1 then
      for r=1 to sdrow
        inps=""
        for c=1 to cols
          if rnd<f then inps+="X" else inps+=" "
        next
        seed(r)=inps
      next
    else
      ? "Sorry, something off with rows, columns or density."
      sdRow=0      
    fi
  else 'menu = fname? well if it exists!
    if exist(menu) then seedfile(menu) else ? "Sorry, "+menu+", file does not exist."
  fi  
end
'======================================================= seedfile
sub seedfile(fname)
