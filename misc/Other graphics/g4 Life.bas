'g4 Life.bas SmallBASIC (not MS) B+ 2015-03-24 this is pretty nice version
...
an=130: s=int(ymax/an): if s<3 then cs=s else cs=s-1: bigBlock=an*s '<==============only set an
dim a(1 to an,1 to an), seed(1 to an), ng(1 to an,1 to an)
waitms=125 '<125 good for an130=======================================according to an, set waitms

'std conways 23/3, crystal 23/23, pagoda 1234/28, Mazectric 1234/4, Ameoba 1358,357

surviveRule="1234" : birthRule="04" '<==================================set SurviveRule and birthRule
...
rndseedfill=.3333 '<=================================set rndSeedFill
...
sub theNextGeneration()
  ...
  for x=2 to an-1
    for y=2 to an-1
      r=str(a(x-1,y-1)+a(x-1,y)+a(x-1,y+1)+a(x,y-1)+a(x,y+1)+a(x+1,y-1)+a(x+1,y)+a(x+1,y+1))
